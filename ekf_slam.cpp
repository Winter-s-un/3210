...
//    initialize ros publisher
    ...
    Q = Eigen::MatrixXd::Identity(2, 2); // measurement noise
...
void EKFSLAM::run() {
    ...
    while (ros::ok()){
		...
		// 2. update
        updateMeasurement(mState, mCov, ut, dt);
		timer.toc();
...
Eigen::MatrixXd EKFSLAM::jacobGt(const Eigen::VectorXd& state, Eigen::Vector2d ut, double dt){
	 ...
	 */
    Gt(0, 2) = -ut(0) * dt * sin(state(2));
    Gt(1, 2) = ut(0) * dt * cos(state(2));
	return Gt;
}
...
void EKFSLAM::addNewLandmark(const Eigen::Vector2d& lm, const Eigen::MatrixXd& InitCov){
	 ...
	 */
     mState.conservativeResize(mState.rows() + 2);
    mCov.conservativeResize(mCov.rows() + 2, mCov.cols() + 2);

    int idx = (mState.rows() - 3) / 2;
    mState.segment(3 + idx * 2, 2) = lm;
    mCov.block(3 + idx * 2, 3 + idx * 2, 2, 2) = InitCov;
}
...
void EKFSLAM::updateMeasurement(Eigen::VectorXd& state, Eigen::MatrixXd& cov, Eigen::Vector2d ut, double dt){
    ...
    for (int i = 0; i < num_obs; ++i) {
		 ...
		 *
		 * **/
        // Implement data association here (nearest neighbor)
        ...
        for (int j = 0; j < num_landmarks; ++j) {
            ...
            double distance = (pt_transformed - landmark).norm();
    ...
    for (int i = 0; i < num_obs; ++i) {
        ...
        Eigen::Vector2d z;
        Eigen::Vector2d observationPoint = cylinderPoints.row(i);
        z(0) = observationPoint.norm();
        z(1) = atan2(observationPoint(1), observationPoint(0));
        // Compute innovation
        Eigen::Vector2d innovation = z.block<2, 1>(2 * i, 0) - transform(landmark, mState.segment(0, 3));

        // Compute Jacobian of measurement model (Ht)
        Eigen::MatrixXd Ht = Eigen::MatrixXd::Zero(2, mState.rows());
        Ht.block<2, 3>(0, 0) = -jacobB(mState, ut, dt).block<2, 3>(0, 0);
        Ht.block<2, 2>(0, 3 + idx * 2) = Eigen::MatrixXd::Identity(2, 2);

        // Compute Kalman gain
        Eigen::MatrixXd K = mCov * Ht.transpose() * (Ht * mCov * Ht.transpose() + Q).inverse();

        // Update state and covariance
        mState += K * innovation;
        mCov = (Eigen::MatrixXd::Identity(mState.rows(), mState.rows()) - K * Ht) * mCov;
    }
    }
}
